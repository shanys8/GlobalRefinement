...
#######################################################################################################################
# Running test data - sampled from continuous function a subdivision using Bspline with m=5
...
def bspline_rules(vals, is_even_indices):
    if is_even_indices:  # even indices
        if np.shape(vals)[0] <= 3: # handle last refinement where f(i+2) is not defined
            one_avg = avg_func(vals[0, :, :], vals[2, :, :], 1 / 2)
            new_val = avg_func(vals[1, :, :], one_avg, 3 / 8)
            return new_val

        one_avg = avg_func(vals[0, :, :], vals[1, :, :], 1/16)
        two_avg = avg_func(vals[3, :, :], vals[2, :, :], 1/16)
        new_val = avg_func(two_avg, one_avg, 1/2)
    else:  # odd indices
        one_avg = avg_func(vals[0, :, :], vals[2, :, :], 1/2)
        new_val = avg_func(vals[1, :, :], one_avg, 3/8)
